...
- **`MaybeSeparateExtensionFromPhone`** (`phonenumberutil.go`) — a nyaruka-only helper
  with no upstream equivalent, retained for compatibility.
- **v2 API cleanups** — the deprecated `FormatWithBuf` and the exported `StringBuilder`
  type remain available for backwards compatibility.
...
- **2026-06-28** — Ported the compatibility-preserving pre-v2 upstream core into
  `github.com/oarkflow/phone` and updated all embedded metadata to `v9.0.33`
  (the metadata shipped by nyaruka/phonenumbers v2.0.2). The local convenience,
  country, and MCC/MNC APIs remain available.
