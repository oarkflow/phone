...
func buildNumberMetadata(srcFile, varName, dstFile string, short bool) (*phone.PhoneMetadataCollection, error) {
	body, err := os.ReadFile("_build/" + srcFile)
	if err != nil {
		return nil, fmt.Errorf("error reading %s: %w", srcFile, err)
	}

	collection, err := phone.BuildPhoneMetadataCollection(body, false, false, short)
	if err != nil {
...
func buildRegionMetadata(metadata *phone.PhoneMetadataCollection, varName, dstFile string) error {
	regionMap := phone.BuildCountryCodeToRegionMap(metadata)
