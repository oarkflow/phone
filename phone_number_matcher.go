...
// PhoneNumberMatcher A stateful class that finds and extracts telephone numbers fom text.
//
...
var (
	// PATTERN The phone number pattern used by {@link #find}, similar to
	...
	// PUB_PAGES Matches strings that look like publication pages. Example:
	...
	// SLASH_SEPARATED_DATES Matches strings that look like dates using "/" as a separator. Examples: 3/10/2011, 31/10/96 or 08/31/95.
	SLASH_SEPARATED_DATES = regexp.MustCompile("(?:(?:[0-3]?\\d/[01]?\\d)|(?:[01]?\\d/[0-3]?\\d))/(?:[12]\\d)?\\d{2}")

	// TIME_STAMPS Matches timestamps. Examples: "2012-01-02 08:00". Note that the reg-ex does not include the trailing ":\d\d" -- that is covered by TIME_STAMPS_SUFFIX.
	TIME_STAMPS        = regexp.MustCompile("[12]\\d{3}[-/]?[01]\\d[-/]?[0-3]\\d +[0-2]\\d$")
	TIME_STAMPS_SUFFIX = regexp.MustCompile(":[0-5]\\d")

	// MATCHING_BRACKETS Pattern to check that brackets match. Opening brackets should be closed within a phone number.
	...
	// INNER_MATCHES Patterns used to extract phone numbers from a larger phone-number-like pattern. These are
	...
	// LEAD_CLASS Punctuation that may be at the start of a phone number - brackets and plus signs.
	LEAD_CLASS   = OPENING_PARENS + PLUS_CHARS
	LEAD_PATTERN = regexp.MustCompile(LEAD_CLASS)

	// OPENING_PARENS Builds the MATCHING_BRACKETS and PATTERN regular expressions. The building blocks below exist to make the pattern more easily understood.
	OPENING_PARENS = "\\(\\[\uFF08\uFF3B"
	CLOSING_PARENS = "\\)\\]\uFF09\uFF3D"
	NON_PARENS     = "[^" + OPENING_PARENS + CLOSING_PARENS + "]"

	// BRACKET_PAIR_LIMIT Limit on the number of pairs of brackets in a phone number.
	BRACKET_PAIR_LIMIT = "{0,3}"

	// LEAD_LIMIT Limit on the number of leading (plus) characters.
	LEAD_LIMIT = "{0,2}"
...
// NewPhoneNumberMatcher Creates a new instance.
//
// Arguments:
